...
| Gate | Result |
| --- | --- |
| Native behavior suite | 255 tests pass on macOS arm64, including isolated real-tmux journeys, mixed-runtime compatibility, and the frozen-updater transition |
...
| Startup | p95 3.92 ms native versus 140.23 ms Python, 100 launches each |
| Board | first-frame p95 10.41 ms versus 458.65 ms; input p95 0.115 ms versus 8.38 ms |
| Hook | p95 18.39 ms versus 473.33 ms, 100 events each |
| Reconciliation | 200-row public path p95 60.92 ms versus 2,398.79 ms, 30 runs each |
| Warm resources | 6.47 MiB RSS p95 and 0.774% one-core mean over a five-minute native board run |
| Artifact | 4.17 MB executable; 2.15 MB gzip; no Python/runtime/compiler prerequisite |
