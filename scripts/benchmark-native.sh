...
if [ -n "$pika_baseline" ]; then
    [ -x "$pika_baseline" ] || { printf 'Baseline is not executable: %s\n' "$pika_baseline" >&2; exit 2; }
    scripts/measure-startup.pl python "$pika_baseline" "$pika_samples" 2>/dev/null
fi
...
[ -z "$pika_baseline" ] || measure_rss python "$pika_baseline"
printf 'os\t%s\nhardware\t%s\n' "$(uname -srv)" "$(uname -m)"
