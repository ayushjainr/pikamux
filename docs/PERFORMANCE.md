...
| --- | ---: | ---: | ---: | ---: |
| CLI startup p95 | 100 each | 4.15 ms | 139.44 ms | ≤30 ms |
| Cached board first-frame p95 | 30 each | 9.91 ms | 473.50 ms | ≤200 ms |
| Board input redraw p95 / p99 | 1,000 each | 0.128 / 0.133 ms | 8.36 / 8.64 ms | ≤50 / 100 ms |
| Hook fast-path p95 | 100 each | 13.89 ms | 511.07 ms | ≤75 ms |
| Full 200-row local reconciliation p95 | 30 each | 69.42 ms | 2,441.08 ms | ≤500 ms |
| Warm board RSS p95 | 300 s native / 60 s Python | 6.92 MiB | 36.42 MiB | ≤40 MiB |
| Native idle board CPU mean | 300 s | 0.630% of one core | — | ≤1% |
| Executable / gzip | one release build | 4.27 / 2.20 MB | interpreter environment required | ≤50 / 20 MiB |

The 100-launch native startup run had a 3.79 ms p50 and 4.28 ms maximum. The
dedicated board run had an 11.68 ms maximum. The five-minute CPU run repainted the clock once per
...
6.94 MiB, RSS p95 was 6.92 MiB, and its second-half CPU mean was 0.684%.
